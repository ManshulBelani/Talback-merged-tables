...
public class WebInterfaceUtils {
    ...
    /**
     * Direction constant for upward movement within a page.
     */
    public static final int DIRECTION_UPWARD = 2;

    /**
     * Direction constant for downward movement within a page.
     */
...
//                : AccessibilityNodeInfoCompat.ACTION_PREVIOUS_HTML_ELEMENT;
        ...
        int action=0;//=AccessibilityNodeInfoCompat.ACTION_NEXT_HTML_ELEMENT;
        if (direction== DIRECTION_FORWARD)
            action = AccessibilityNodeInfoCompat.ACTION_NEXT_HTML_ELEMENT;
        else    if (direction== DIRECTION_BACKWARD)
           action = AccessibilityNodeInfoCompat.ACTION_PREVIOUS_HTML_ELEMENT;
        else if (direction== DIRECTION_UPWARD) {
            action = CursorControllerApp.HTML_UP;
        }
        else //if (direction== DIRECTION_DOWNWARD);
            action = CursorControllerApp.HTML_DOWN;
