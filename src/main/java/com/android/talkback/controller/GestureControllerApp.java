    ...
    public void performAction(String action) {
        ...
        if (action.equals(mService.getString(R.string.shortcut_value_unassigned))) {
          // Do Nothing
            ...
            boolean result = mCursorController.upHTML(
                    true /*useInputFocusAsPivotIfEmpty*/,
                    InputModeManager.INPUT_MODE_TOUCH);

            // if result is false , it means we are not in a table
            // so we should change granularity of speaking
            if (!result) {
                boolean newResult = mCursorController.previousGranularity();
                if (!newResult) mFeedbackController.playAuditory(R.raw.complete);
            }
        } else if (action.equals(mService.getString(R.string.shortcut_value_next_granularity))) {
            boolean result = mCursorController.downHTML(
                    true /*useInputFocusAsPivotIfEmpty*/,
                    InputModeManager.INPUT_MODE_TOUCH);
            ...
            // so we should change granularity of speaking
            if (!result) {
                boolean newResult = mCursorController.nextGranularity();
                if (!newResult) mFeedbackController.playAuditory(R.raw.complete);
            }
